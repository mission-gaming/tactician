...
- **Time-aware rules shipped** (`TimelineRule`: `MinimumRestRule`, `BlackoutRule`) — loud post-assignment validation, deliberately not generation constraints. **Resources shipped too**: named resources on the timeline lift the one-event-per-slot default (concurrent kickoffs, deterministic resource assignment on each `ScheduledEvent`). Phase 4's designed scope is complete (demand-gated leftovers: cross-stage clash validation, per-resource availability windows). **Phase 5 is underway**: backtracking round-robin generation shipped (`docs/design/backtracking-generation.md` — opt-in, deterministic, step-bounded; closes the longest-recorded known limitation). Quality metrics + best-of-N optimization also shipped (`src/Quality/`, design note `docs/design/schedule-quality.md`). Constraint attribution diagnostics also shipped (probing analysis attached to generation failures, design note `docs/design/diagnostics-attribution.md`). **Phase 5 is complete**: framework integration guides shipped (docs/integrations/ — Symfony as the Metronome-shaped centrepiece, Laravel mirror, example 18 for the stateless request-cycle pattern). All five roadmap phases are now ✅; future work is demand-gated (cross-stage clash validation, per-resource availability windows, smarter optimization algorithms behind the existing scorer).
